...
public:
	...
	void createPSO(DX12Core* core, std::string pso_name, ID3DBlob* vs, ID3DBlob* ps, D3D12_INPUT_LAYOUT_DESC layout) {
		...
		// Blending
		// Combining the color of a pixel being drawn (the source)
		// with the color already present in the render target(the destination)
		...
		psos.insert({ pso_name, pso });
	}

	// Bind based on name
	void bind(DX12Core* core, std::string pso_name) {
		core->getCommandList()->SetPipelineState(psos[pso_name]);
	}
};